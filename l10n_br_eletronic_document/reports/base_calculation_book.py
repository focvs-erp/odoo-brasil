...
class ReportCalculationBook(models.AbstractModel):
    """
    Este modelo será usado como base para realizar os calculos dos livros de apuração IPI e ICMS.
    icms_book
    ipi_book
    """
    ...
    def _get_report_values(self, docids, data=None):
        ...
        entry_notes_by_cfop = self.calculate_total_by_cfop(
            invoices=self.get_invoices_by_operation_type(docs=docs, operation_type='entrada'), headers=self.HEADERS)

        exit_notes_by_cfop = self.calculate_total_by_cfop(
            invoices=self.get_invoices_by_operation_type(docs=docs, operation_type='saida'), headers=self.HEADERS)
        ...
        return {
            ...
            'grouped_by_cfop_type_entry_notes': self.group_values_by_cfop_type(invoices=entry_notes_by_cfop, headers=self.HEADERS),
            'total_entry_notes': self.calculate_total_for_each_attribute(invoices=entry_notes_by_cfop, headers=self.HEADERS),
            # Exit notes
            'exit_notes_by_cfop': exit_notes_by_cfop,
            'grouped_by_cfop_type_exit_notes': self.group_values_by_cfop_type(invoices=exit_notes_by_cfop, headers=self.HEADERS),
            'total_exit_notes': self.calculate_total_for_each_attribute(invoices=exit_notes_by_cfop, headers=self.HEADERS),
        }
